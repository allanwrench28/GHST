...
class PluginManager:
    ...
    def __init__(self, plugin_dirs: List[str] = None):
        ...
        if plugin_dirs is None:
            plugin_dirs = [
                ...
                str(Path(__file__).parent.parent.parent / "plugins"),  # User plugins
        ...
        self.logger.warning("⚠️ Plugin system active - plugins run with system access!")
    ...
    def discover_plugins(self) -> List[str]:
        ...
        for plugin_dir in self.plugin_dirs:
            ...
            for py_file in plugin_path.glob("*.py"):
                ...
                discovered.append(f"{plugin_dir}:{module_name}")
                
            # Look for plugin packages
            for pkg_dir in plugin_path.iterdir():
                if pkg_dir.is_dir() and (pkg_dir / "__init__.py").exists():
                    discovered.append(f"{plugin_dir}:{pkg_dir.name}")
    ...
    def load_plugin(self, plugin_spec: str) -> bool:
        ...
        try:
            ...
            # Also add the parent plugins directory so base_plugin can be imported
            ...
            try:
                ...
                # Find plugin classes - look for classes that inherit from our BasePlugin
                plugin_classes = []
                for name, obj in inspect.getmembers(module, inspect.isclass):
                    ...
                    if (name.endswith('Plugin') and 
                        ...
                        for base in obj.__bases__:
                            if base is BasePlugin or (hasattr(base, '__name__') and base.__name__ == 'BasePlugin'):
                ...
                if BasePlugin not in plugin_class.__bases__:
                    self.logger.debug(f"Fixing inheritance for {plugin_class.__name__}")
                    # Create a new class with the correct base
                    new_class = type(plugin_class.__name__, (BasePlugin,), dict(plugin_class.__dict__))
                ...
                if not plugin_instance.is_compatible():
                    self.logger.warning(f"Plugin {plugin_instance.metadata.name} is not compatible")
                    return False
                ...
                if plugin_instance.initialize():
                    ...
                    self.logger.info(f"✅ Loaded plugin: {plugin_instance.metadata.name} v{plugin_instance.metadata.version}")
                    
                    # Show safety warning for experimental plugins
                    if plugin_instance.metadata.experimental:
                        self.logger.warning(f"⚠️ EXPERIMENTAL plugin loaded: {plugin_instance.metadata.name}")
                    
                    return True
                else:
                    self.logger.error(f"Failed to initialize plugin: {plugin_instance.metadata.name}")
                    return False
            ...
            self.logger.error(f"Failed to load plugin {plugin_spec}: {e}")
            import traceback
    ...
    def process_mesh_through_plugins(self, mesh_data: Any) -> Any:
        ...
        for plugin in self.get_plugins_by_category("slicer"):
            try:
                ...
                self.logger.error(f"Error processing mesh with plugin {plugin.metadata.name}: {e}")
        return mesh_data
    ...
    def process_gcode_through_plugins(self, gcode_lines: List[str]) -> List[str]:
        ...
        for plugin in self.get_plugins_by_category("slicer"):
            try:
                ...
                self.logger.error(f"Error processing G-code with plugin {plugin.metadata.name}: {e}")
        return gcode_lines
    ...
    def get_all_menu_actions(self) -> List[Dict[str, Any]]:
        ...
        for plugin in self.get_enabled_plugins():
            try:
                ...
                self.logger.error(f"Error getting menu actions from plugin {plugin.metadata.name}: {e}")
        return actions
