...
for import_path in [
    ...
    try:
        module = __import__(import_path, fromlist=['BasePlugin', 'PluginMetadata'])
...
class PrintStatsPlugin(BasePlugin):
    ...
    def on_slicing_completed(self, gcode_lines: List[str], stats: Dict[str, Any]):
        """Calculate statistics when slicing completes."""
        try:
            self.stats = self._calculate_stats(gcode_lines, stats)
            self.logger.info(f"Calculated print statistics: {self.stats}")
        except Exception as e:
            self.logger.error(f"Failed to calculate statistics: {e}")
    
    def _calculate_stats(self, gcode_lines: List[str], basic_stats: Dict[str, Any]) -> Dict[str, Any]:
        """Calculate detailed statistics from G-code."""
        stats = {
            'total_lines': len(gcode_lines),
            'estimated_time_minutes': basic_stats.get('estimated_print_time', 0),
            'layer_count': basic_stats.get('layers', 0),
        ...
        extrusion_moves = sum(1 for line in gcode_lines if line.startswith('G1') and 'E' in line)
        stats['estimated_filament_length'] = extrusion_moves * 0.5  # mm
        stats['estimated_filament_weight'] = stats['estimated_filament_length'] * 0.00125  # grams
        stats['estimated_cost'] = stats['estimated_filament_weight'] * 0.02  # $0.02/gram
    ...
    def show_statistics(self):
        ...
        if not self.stats:
            self.logger.warning("No statistics available - slice a model first")
            return
