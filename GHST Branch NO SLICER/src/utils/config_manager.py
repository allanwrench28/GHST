...
class ConfigManager:
    ...
    def __init__(self, config_dir: str = "config"):
        ...
        if PLUGINS_AVAILABLE:
            try:
                ...
                self.logger.info(f"Plugin system enabled with {len(self.plugin_manager.loaded_plugins)} plugins loaded")
            except Exception as e:
                self.logger.warning(f"Failed to initialize plugin manager: {e}")
        else:
    ...
    def save_yaml_config(self, config_data: Dict[str, Any], filename: str):
        ...
        try:
            with open(file_path, 'w', encoding='utf-8') as f:
                ...
                f.write("# FANTOM Configuration File\n")
    ...
    def load_printer_config(self, config_name: str) -> PrinterConfig:
        ...
        try:
            config_data = self.load_yaml_config(f"{config_name}.yaml")
        except FileNotFoundError:
            # Create default config if file doesn't exist
            self.logger.warning(f"Printer config {config_name}.yaml not found, using defaults")
    ...
    def load_material_config(self, config_name: str) -> MaterialConfig:
        ...
        try:
            ...
            self.logger.warning(f"Material config {config_name}.yaml not found, using defaults")
    ...
    def load_slicing_config(self, config_name: str) -> SlicingConfig:
        ...
        try:
            ...
            self.logger.warning(f"Slicing config {config_name}.yaml not found, using defaults")
    ...
    def enable_experimental_feature(self, feature: str, enabled: bool = True):
        ...
        # Log safety warning
        if enabled:
            self.logger.warning(f"⚠️  Experimental feature '{feature}' ENABLED - Use at your own risk!")
        else:
            self.logger.info(f"Experimental feature '{feature}' disabled")
    ...
    def enable_plugin(self, plugin_name: str) -> bool:
        ...
        if result:
            self.logger.warning(f"⚠️ Plugin '{plugin_name}' ENABLED - Use at your own risk!")
        return result
    ...
    def process_gcode_with_plugins(self, gcode_lines: List[str]) -> List[str]:
        ...
        if self.plugin_manager:
            return self.plugin_manager.process_gcode_through_plugins(gcode_lines)
        return gcode_lines
        ...
        if self.current_printer:
            ...
            if hasattr(self.current_material, 'temp_hotend'):
                if self.current_material.temp_hotend > self.current_printer.max_temp_hotend:
                    warnings.append(f"⚠️  Material hotend temp ({self.current_material.temp_hotend}°C) exceeds printer max ({self.current_printer.max_temp_hotend}°C)")
                    
            if hasattr(self.current_material, 'temp_bed'):
                if self.current_material.temp_bed > self.current_printer.max_temp_bed:
                    warnings.append(f"⚠️  Material bed temp ({self.current_material.temp_bed}°C) exceeds printer max ({self.current_printer.max_temp_bed}°C)")
                    
        if self.current_slicing:
            # Check for experimental features
            enabled_experimental = [feature for feature, enabled 
                                  in self.current_slicing.experimental_features.items() 
                                  if enabled]
            
            if enabled_experimental:
                warnings.append(f"⚠️  Experimental features enabled: {', '.join(enabled_experimental)}")
                
            # Check for non-planar slicing
            if self.current_slicing.experimental_features.get("nonplanar_slicing", False):
                if not self.current_printer or not self.current_printer.supports_nonplanar:
                    warnings.append("🚨 Non-planar slicing enabled but printer may not support it!")
    ...
    def export_config_bundle(self, output_path: str):
        ...
        bundle = {
            ...
            'printer': asdict(self.current_printer) if self.current_printer else None,
            'material': asdict(self.current_material) if self.current_material else None,
            'slicing': asdict(self.current_slicing) if self.current_slicing else None
        }
        
        with open(output_path, 'w') as f:
            yaml.dump(bundle, f, default_flow_style=False, indent=2)
            
        self.logger.info(f"Exported configuration bundle: {output_path}")
    ...
    def get_config_summary(self) -> str:
        ...
        if self.current_printer:
            ...
            summary.append(f"   Bed: {self.current_printer.bed_size_x}×{self.current_printer.bed_size_y}×{self.current_printer.bed_size_z}mm")
            summary.append(f"   Nozzle: {self.current_printer.nozzle_diameter}mm\n")
            
        if self.current_material:
            summary.append(f"🧱 Material: {self.current_material.name} ({self.current_material.type})")
            summary.append(f"   Hotend: {self.current_material.temp_hotend}°C, Bed: {self.current_material.temp_bed}°C\n")
            
        if self.current_slicing:
            summary.append(f"⚡ Slicing: {self.current_slicing.layer_height}mm layers, {self.current_slicing.infill_percentage}% infill")
            
            enabled_experimental = [feature for feature, enabled 
                                  in self.current_slicing.experimental_features.items() 
                                  if enabled]
            if enabled_experimental:
                summary.append(f"🧪 Experimental: {', '.join(enabled_experimental)}")
