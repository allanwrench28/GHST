            ...
            for cache_dir in self.cache_dirs:
                if cache_dir in dirs:
                    ...
                    try:
                        ...
                        cleaned.append(f"Removed cache: {cache_path.relative_to(self.install_path)}")
                    except Exception as e:
                        cleaned.append(f"Warning: Could not remove {cache_path}: {e}")
        ...
        for root, dirs, files in os.walk(self.install_path):
            for file in files:
                if file.endswith('.pyc') or file.endswith('.pyo'):
                    ...
                    try:
                        pyc_path.unlink()
                        cleaned.append(f"Removed: {pyc_path.relative_to(self.install_path)}")
                    except Exception as e:
                        cleaned.append(f"Warning: Could not remove {pyc_path}: {e}")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                return {
                    ...
                    'assets': [asset['browser_download_url'] for asset in release_data.get('assets', [])]
                }
        except Exception as e:
            return {
                'available': False,
...
class InstallationWorker(QThread):
        ...
        try:
            ...
            if self.install_options.get('clean_install', True):
                ...
                if preferences_backup:
                    self.log_updated.emit(f"✅ Backed up {len(preferences_backup)} preference files")
                
                # Clean caches and temporary files
                self.status_updated.emit("Cleaning caches and temporary files...")
                ...
                if len(cleaned_items) > 10:
                    self.log_updated.emit(f"🧹 ... and {len(cleaned_items) - 10} more items")
            ...
            self.log_updated.emit(f"❌ Installation failed: {str(e)}")
    ...
    def install_dependencies(self):
        ...
        for i, package in enumerate(packages):
            ...
            self.log_updated.emit(f"📦 Installing {package}...")
            ...
            try:
                ...
                self.log_updated.emit(f"✅ {package} installed successfully")
                
            except subprocess.CalledProcessError as e:
                self.log_updated.emit(f"⚠️ {package} installation failed: {e}")
            except Exception as e:
                self.log_updated.emit(f"❌ Installation error: {str(e)}")
    ...
    def setup_environment(self):
        ...
        for directory in ghst_dirs:
            ...
            self.log_updated.emit(f"📁 Created directory: {directory}")
...
class UpdateTab(QWidget):
    ...
    def init_ui(self):
        ...
        self.current_version_label = QLabel(f"Version: {CURRENT_VERSION}")
        ...
        self.update_status = QLabel("Click 'Check for Updates' to see if newer versions are available.")
    ...
    def update_check_finished(self):
        ...
        if 'error' in self.update_info:
            self.update_status.setText(f"❌ Update check failed: {self.update_info['error']}")
            return
        ...
        if self.update_info['available']:
            ...
            self.update_status.setStyleSheet("color: #28a745; font-weight: bold;")
            
            # Show release notes
            self.update_details.setPlainText(self.update_info.get('release_notes', 'No release notes available'))
    ...
    def open_download_page(self):
        ...
        import webbrowser
        download_url = self.update_info.get('download_url', f'https://github.com/{GITHUB_REPO}/releases')
...
class WelcomePage(QWidget):
    ...
    def init_ui(self):
        ...
        welcome_text.setStyleSheet("font-size: 12px; line-height: 1.4; margin: 20px;")
...
class OptionsPage(QWidget):
    ...
    def init_ui(self):
        ...
        self.clean_install = QCheckBox("🧹 Perform clean installation (recommended)")
        self.clean_install.setChecked(True)
        self.clean_install.setToolTip("Remove caches and temporary files, but preserve user preferences")
        ...
        self.dev_mode.setToolTip("Install additional packages for GHST development")
...
class CompletionPage(QWidget):
    ...
    def init_ui(self):
        ...
        title.setStyleSheet("font-size: 24px; font-weight: bold; color: #28a745; margin: 20px;")
        title.setAlignment(Qt.AlignCenter)
        layout.addWidget(title)
        ...
        next_steps.setStyleSheet("font-size: 12px; line-height: 1.5; margin: 20px;")
        ...
        self.launch_button.setStyleSheet("background-color: #0078d4; color: white; font-weight: bold; padding: 10px;")
...
class GHSTInstaller(QMainWindow):
    ...
    def init_ui(self):
        self.setWindowTitle("GHST AI Coding Engine - Enhanced Installer v1.0.0-alpha.4")
    ...
    def go_next(self):
        ...
        if self.current_page == 1:  # Options page
            ...
            self.tab_widget.setCurrentIndex(self.current_page)
            # Start installation
            self.installation_page.start_installation(self.installation_options)
