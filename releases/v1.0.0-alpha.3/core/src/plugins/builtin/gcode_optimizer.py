...
class GcodeOptimizerPlugin(BasePlugin):
    ...
    def initialize(self) -> bool:
        ...
        self.logger.warning("⚠️ G-code Optimizer plugin initialized - EXPERIMENTAL!")
    ...
    def process_gcode(self, gcode_lines: List[str]) -> List[str]:
        ...
        for line in gcode_lines:
            ...
            if line.startswith('G1') and 'X' not in line and 'Y' not in line and 'Z' not in line:
        ...
        self.logger.info(f"G-code optimization complete: removed {reduction} lines ({reduction/len(gcode_lines)*100:.1f}%)")
    ...
    def toggle_optimization(self):
        ...
        self.logger.warning(f"⚠️ G-code optimization {status}")
        print(f"G-code optimization {status}")
