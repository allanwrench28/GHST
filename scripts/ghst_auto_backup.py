...
class GHSTBackupManager:
    ...
    def __init__(self, workspace_path: str):
        ...
        self.session_backup = self.backup_root / f"session_{self.timestamp}"
        self.session_backup.mkdir(exist_ok=True)
        
        print(f"🔒 Backup Manager initialized")
        print(f"📁 Workspace: {self.workspace_path}")
        print(f"💾 Backup location: {self.session_backup}")
    ...
    def create_full_backup(self):
        ...
        try:
            ...
            print(f"✅ Full directory backup: {full_backup}")
        ...
        try:
            ...
            print(f"✅ Critical files backup: {critical_backup}")
        except Exception as e:
            print(f"❌ Critical files backup failed: {e}")
        
        # 5. Create backup manifest
        self._create_backup_manifest(backups_created)
        
        print(f"\n🎉 Backup process complete!")
        print(f"📊 {len(backups_created)} backup types created")
        print(f"📋 Manifest saved to: {self.session_backup / 'backup_manifest.json'}")
    ...
    def _create_zip_backup(self):
        ...
        zip_path = self.session_backup / f"ghst_backup_{self.timestamp}.zip"
        
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zipf:
            for root, dirs, files in os.walk(self.workspace_path):
                # Skip unwanted directories
                dirs[:] = [d for d in dirs if d not in [
                    '__pycache__', '.git', 'node_modules', '.vscode', 'GHST_BACKUPS'
                ]]
    ...
    def _create_git_backup(self):
        ...
        try:
            # Create git bundle
            bundle_path = self.session_backup / f"ghst_git_{self.timestamp}.bundle"
            ...
            subprocess.run([
                ...
                f'--output={self.session_backup / f"git_snapshot_{self.timestamp}.zip"}',
                'HEAD'
            ], cwd=self.workspace_path, check=True, capture_output=True)
    ...
    def verify_backups(self):
        ...
        for backup in manifest['backups']:
            ...
            if backup_path.exists():
                print(f"✅ {backup['type']}: {backup['size_mb']} MB")
            else:
                print(f"❌ {backup['type']}: NOT FOUND")
                all_good = False
...
def main():
    ...
    if backup_manager.verify_backups():
        ...
        print(f"📁 Backups stored at: {backup_location}")
        return True
    else:
