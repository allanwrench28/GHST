...
class SmartAutoCommit:
                    ...
                    if current_batch:
                        batches.append(
                            CommitBatch(
                                ...
                                description=self.generate_batch_description(
                                    category,
                                    current_batch),
                                category=category))
    ...
    def run_auto_commit(self) -> bool:
        ...
        response = input(
            f"\n🚀 Proceed with {
                len(batches)} commits? (y/N): ").strip().lower()
        if response != 'y':
        ...
        if success:
            print(
                f"\n🎉 Auto-commit complete! {successful_batches}/{
                    len(batches)} batches committed and pushed")
        else:
            print(
