        ...
        try:
            ...
            try:
                ...
                if not plugin_classes:
                    self.logger.warning(
                        f"No plugin classes found in {module_name}")
                    return False
                ...
                if plugin_instance.initialize():
                    ...
                    if plugin_instance.metadata.experimental:
                        self.logger.warning(
                            f"⚠️ EXPERIMENTAL plugin loaded: {
    ...
    def call_plugin_hook(self, hook_name: str, *args, **kwargs):
        ...
        for plugin in self.get_enabled_plugins():
            try:
                ...
                self.logger.error(
                    f"Error calling {hook_name} on plugin {
    ...
    def process_mesh_through_plugins(self, mesh_data: Any) -> Any:
        ...
        for plugin in self.get_plugins_by_category("slicer"):
            try:
                ...
                self.logger.error(
                    f"Error processing mesh with plugin {
                        plugin.metadata.name}: {e}")
        return mesh_data
    ...
    def process_gcode_through_plugins(
        ...
        for plugin in self.get_plugins_by_category("slicer"):
            try:
                ...
                self.logger.error(
                    f"Error processing G-code with plugin {plugin.metadata.name}: {e}")
        return gcode_lines
    ...
    def get_all_menu_actions(self) -> List[Dict[str, Any]]:
        ...
        for plugin in self.get_enabled_plugins():
            try:
                ...
                self.logger.error(
                    f"Error getting menu actions from plugin {
                        plugin.metadata.name}: {e}")
        return actions
