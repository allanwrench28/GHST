...
class ErrorHandler:
    ...
    def start_processing(self):
        ...
        if self.ghost_manager:
            self.ghost_manager.log_activity(
                "🚨 Error handler started - Ghost analysis enabled")
        ...
        try:
            ...
            self.logger.error(
                f"Captured {
                    ...
                    'ghost_analysis': 'Queued for analysis'})

            if self.ghost_manager:
                self.ghost_manager.log_activity(
                    f"🚨 Error captured: {
                        error_data['exception_type']} - Ghost analysis queued")
            ...
            f"Error Type: {
                error_data.get(
                    'exception_type', error_data.get(
                        'error_code', 'Unknown'))}", f"Category: {
                    error_data.get(
                        'category', 'Unknown')}", f"Severity: {
                            error_data.get(
                                'severity', 'Unknown')}", f"Context: {
                                    error_data.get(
    ...
    def _submit_ghost_fix(
        ...
        if not self.ghost_manager or not hasattr(
                self.ghost_manager, 'submit_ghost_pr'):
            return
        ...
        try:
            ...
            if success:
                self.ghost_manager.log_activity(
                    f"📝 Ghost fix submitted for {
                        error_data.get(
                            'error_id', 'unknown')}")
            else:
                self.ghost_manager.log_activity(
                    f"❌ Ghost fix submission failed for {
                        error_data.get(
                            'error_id', 'unknown')}")
            ...
            if self.ghost_manager:
                self.ghost_manager.log_activity(
                    f"❌ Ghost fix submission error: {e}")
...
class SafetyChecker:
    ...
    def verify_temperature_settings(hotend_temp, bed_temp, material_type):
        ...
        if not (limits['hotend'][0] <= hotend_temp <= limits['hotend'][1]):
            raise ValueError(f"Unsafe hotend temperature for {material_type}: {hotend_temp}°C")

        if not (limits['bed'][0] <= bed_temp <= limits['bed'][1]):
            raise ValueError(f"Unsafe bed temperature for {material_type}: {bed_temp}°C")
...
'''
    ...
    def _generate_error_id(self, exception: Exception, context: str) -> str:
        ...
        error_string = f"{type(exception).__name__}:{str(exception)}:{context}"
    ...
    def _classify_custom_error(self, error_code: str, message: str) -> str:
        ...
        error_text = f"{error_code} {message}".lower()
