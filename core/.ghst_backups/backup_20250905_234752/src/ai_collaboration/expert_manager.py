...
class ExpertManager:
    ...
    def request_commit_approval(self, commit_suggestion):
        ...
        self.log_activity(
            f"   Files: {len(commit_suggestion['files'])} changed")
    ...
    def submit_ghost_pr(self, ghost_id: str, fix_description: str,
        ...
        if not self.github_token:
            self.log_activity(
                f"❌ Cannot submit PR - No GitHub token configured")
            return False
...
**Use at your own risk. Verify all changes before deployment.**
            ...
            self.log_activity(
                f"📝 {ghost_id} submitted PR: {fix_description[:50]}...")
    ...
    def monitor_cycle(self):
        ...
        if hasattr(self.manager, '_current_analysis_needed'):
            self.manager.log_activity(
                f"🔍 {self.ghost_id}: Analyzing mesh quality...")
            # Perform mesh analysis
            time.sleep(2)  # Simulate analysis time
            self.manager.log_activity(
                f"✅ {self.ghost_id}: Mesh analysis complete")
    ...
    def monitor_cycle(self):
        ...
        if self.manager.internet_enabled:
            ...
            if time.time() % 300 < 30:  # Every 5 minutes
                self.manager.log_activity(
                    f"⚡ {self.ghost_id}: Researching optimization techniques...")
...
class ErrorGhost(BaseGhost):
    ...
    def monitor_cycle(self):
        ...
        try:
            ...
            self.manager.log_activity(
                f"❌ {self.ghost_id}: Error monitoring failed: {e}")
...
class ResearchGhost(BaseGhost):
    ...
    def monitor_cycle(self):
        ...
        if time.time() % 600 < 30:  # Every 10 minutes
            ...
            self.manager.log_activity(
                f"📖 {self.ghost_id}: Researching {topic}...")
...
class BackgroundTaskGhost(BaseGhost):
    ...
    def promote_idea_to_task(self, idea_index):
        ...
        if 0 <= idea_index < len(self.idea_queue):
            ...
            self.manager.log_activity(
                f"🚀 Promoted idea to active task: {idea['text'][:50]}...")
            return task
        return None
...
class GitOpsGhost(BaseGhost):
    ...
    def suggest_commit(self, files_changed=None):
        ...
        try:
            ...
            self.manager.log_activity(
                f"🔄 Dr. Git: Error checking repository - {e}")
            return None
    ...
    def execute_commit(self, commit_data, approved=False):
        ...
        try:
            ...
            self.manager.log_activity(
                f"✅ Dr. Git: Successfully committed - {commit_data['message']}")
            return True
...
'''
        ...
        self.manager.log_activity(
            f"🔧 Dr. CICD: Generated fix for {issue_type}")
        return fix_code
