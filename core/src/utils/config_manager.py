...
class ConfigManager:
    ...
    def validate_config_safety(self) -> List[str]:
        ...
        if self.current_slicing:
            ...
            if enabled_experimental:
                warnings.append(
                    f"⚠️  Experimental features enabled: {', '.join(enabled_experimental)}")
    ...
    def get_config_summary(self) -> str:
        ...
        if self.current_printer:
            ...
            summary.append(
                f"   Bed: {self.current_printer.bed_size_x}×{self.current_printer.bed_size_y}×{self.current_printer.bed_size_z}mm")
            summary.append(
        ...
        if self.current_slicing:
            ...
            if enabled_experimental:
                summary.append(
                    f"🧪 Experimental: {', '.join(enabled_experimental)}")
