 ...
 */''',
    ...
    def check_file(self, filepath: str) -> Dict[str, Any]:
        ...
        if file_path.name in self.excluded_files:
            return {'file': filepath, 'status': 'skipped', 'reason': 'excluded_file'}
        
        if any(part in self.excluded_dirs for part in file_path.parts):
            return {'file': filepath, 'status': 'skipped', 'reason': 'excluded_directory'}
        
        # Determine file type
        file_type = self._get_file_type(file_path)
        if not file_type:
            return {'file': filepath, 'status': 'skipped', 'reason': 'unsupported_type'}
        ...
        has_license = bool(re.search(template['pattern'], content, re.DOTALL | re.IGNORECASE))
    ...
    def generate_report(self, results: List[Dict[str, Any]]) -> str:
        ...
        files_missing_license = len([r for r in results if r.get('missing_license')])
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Check and fix license headers')
    parser.add_argument('files', nargs='+', help='Files to check')
    parser.add_argument('--fix', action='store_true', help='Fix missing license headers')
