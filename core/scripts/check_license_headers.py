...
"""
        ...
        if files_missing_license > 0:
            ...
            for result in results:
                if result.get('missing_license'):
                    report += "  ❌ {result['file']} ({result['file_type']})\n"
            report += "\n"
        ...
        if files_missing_license == 0:
            ...
            report += "❌ {files_missing_license} files missing license headers\n"
        ...
        if os.path.exists(filepath):
            ...
            if args.fix and result.get('missing_license'):
                ...
                if file_type:
                    ...
                    if success:
                        result['fixed'] = True
                        print("✅ Added license header to {filepath}")
                    else:
                        result['fix_failed'] = True
                        print("❌ Failed to add license header to {filepath}")
