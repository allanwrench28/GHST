...
class CodeAnalyzer:
    ...
    def analyze_python_file(self, filepath: Path) -> Dict:
        ...
        try:
            ...
            # Calculate scores
            analysis['ml_potential'] = self._calculate_ml_potential(
                content, analysis)
            analysis['innovation_score'] = self._calculate_innovation_score(
                content)
    ...
    def _calculate_ml_potential(self, content: str, analysis: Dict) -> float:
        ...
        ml_matches = sum(
            1 for keyword in self.ml_keywords if keyword in content_lower)
        score += ml_matches * 10

        # Check for data processing patterns
        if any(
            term in content_lower for term in [
                'data',
                'process',
                'analyze',
                'pattern']):
...
class CodebaseCleanupDaemon:
    ...
    def __init__(self, config_file='cleanup_daemon_config.json'):
        ...
        self.check_interval = self.config.get(
            'check_interval_hours', 24) * 3600
        self.analyzer = CodeAnalyzer()
        self.running = False

        # Thresholds
        self.obsolete_threshold_days = self.config.get(
    ...
    def _generate_ml_recommendation(self, analysis: Dict) -> str:
        ...
        if analysis['ml_potential'] > 70:
            recommendations.append(
                "High ML potential - integrate into think tank system")
        elif analysis['ml_potential'] > 50:
            recommendations.append("Good ML candidate - extract algorithms")
        else:
            recommendations.append(
                "ML patterns detected - review for enhancement")

        if analysis['innovation_score'] > 60:
            recommendations.append(
                "Contains innovative patterns worth preserving")
    ...
    for opportunity in sorted(ml_opportunities,
        ...
        f.write(
            f"- **Innovation Score**: {opportunity['innovation_score']}/100\n")
    ...
    def run_cleanup_cycle(self):
        ...
        logger.info(
            f"🤖 Found {len(analysis['ml_opportunities'])} ML opportunities")
        logger.info(
            f"💡 Found {len(analysis['innovation_preserves'])} innovative files")
        logger.info(
            f"🗑️ Found {len(analysis['obsolete_candidates'])} obsolete candidates")

        # Archive obsolete files
        if analysis['obsolete_candidates']:
            archived = self.archive_obsolete_files(
                analysis['obsolete_candidates'])
    ...
    def _generate_summary_report(self, analysis: Dict):
        ...
        with open(report_path, 'w', encoding='utf-8') as f:
            ...
            f.write(
                f"- **ML Opportunities**: {len(analysis['ml_opportunities'])}\n")
            f.write(
                f"- **Innovation Preserves**: {len(analysis['innovation_preserves'])}\n")
            f.write(
                f"- **Files Archived**: {len(analysis['obsolete_candidates'])}\n\n")

            if analysis['ml_opportunities']:
                f.write("## 🤖 Top ML Integration Opportunities\n\n")
                for opp in analysis['ml_opportunities'][:5]:
                    f.write(
                        f"- `{Path(opp['file']).name}` (ML: {opp['ml_potential']}/100)\n")
                f.write("\n")
            ...
            if analysis['innovation_preserves']:
                ...
                for preserve in analysis['innovation_preserves'][:5]:
                    f.write(
                        f"- `{
                            Path(
                                preserve['file']).name}` (Innovation: {
                            preserve['innovation_score']}/100)\n")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='GHST Intelligent Cleanup Daemon')
    ...
    if args.once:
        if args.dry_run:
            ...
            if analysis['ml_opportunities']:
                daemon.generate_ml_integration_report(
                    analysis['ml_opportunities'])
        else:
