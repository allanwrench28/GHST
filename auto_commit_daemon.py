...
class AutoCommitDaemon:
        ...
        for category, files in categories.items():
            ...
            for status, filepath in files:
                ...
                if file_size > self.max_batch_size_mb:
                    logger.warning(f"⚠️ Large file {filepath} ({file_size:.1f}MB) - skipping")
                    continue
    ...
    def commit_batch(self, category, files, size_mb):
        ...
        try:
            ...
            template = self.config['commit_message_templates'].get(category, 
                                                                 '📝 Auto-commit: Changes')
            ...
            logger.info(f"✅ Committed {category} batch: {file_count} files ({size_mb:.1f}MB)")
    ...
    def process_changes(self):
        ...
        logger.info(f"🎯 Successfully committed {success_count}/{len(batches)} batches")
        ...
        try:
            while self.running:
                try:
                    ...
                    # Wait for next check
                    logger.debug(f"😴 Sleeping for {self.check_interval/60:.1f} minutes")
